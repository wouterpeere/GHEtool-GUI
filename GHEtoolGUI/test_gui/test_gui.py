...
def test_language(qtbot):
    ...
    qtbot: qtbot
        bot for the GUI
    """
    from GHEtoolGUI.translation_csv_to_py import main
...
def test_backward_compatibility(qtbot):
    ...
    assert not main_window_old._load_from_data(f'{FOLDER}/test_gui/test_file_version_2_1_0.GHEtool')

    assert main_window_old._load_from_data(f'{FOLDER}/test_gui/test_file_version_2_1_1.GHEtool')
    # init gui window
    main_window_new = start_tests(qtbot)
    assert main_window_new._load_from_data(f'{FOLDER}/test_gui/test_file_version_2_2_0.GHEtool')
